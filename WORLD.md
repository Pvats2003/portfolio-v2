...
- **Sep 26, new direction: look around and click, no walking.** Two prototypes to compare:
  - **`/world`: 360° viewer.** A CSS 3D cube of six painted faces per viewpoint (`components/world/pano/`, faces from `npm run world:pano`), no WebGL or 3D library. Drag/swipe to look, scroll/pinch/± to zoom (30–95°), arrow keys, opt-in device tilt on phones (iOS permission prompt), hotspots anchored in 3D (inn → projects; resume and contact placeholders → their existing links), and arrows on the ground to hop between three viewpoints (village square, inn doorstep, footbridge). The panoramas are **stand-ins** painted from the land plate until real 360° images arrive (`art/pano/README.md`).
  - **`/world/depth`: 3D photo test.** The land plate as a WebGL mesh displaced by a depth map (Depth Anything V2 Small, `npm run world:depth`); mouse, finger drag or tilt shifts the view slightly; the inn outline is projected through the same camera and stays clickable. Off under reduced motion.
- **Scene first (Sep 26):** looking is limited to 25° below the horizon (24° on phones, where the screen's bottom edge is kept no lower than 60° down) and 60° up, and each viewpoint opens a few degrees above level, so the painted floor stays a band at the bottom. Places are small glowing markers with a soft pulse (no pulse under reduced motion) and their name shows on hover or focus; on phones the first tap shows the name and the second opens it. Hop arrows are small, with the name on hover or focus. One corner menu (first in the tab order, "Menu: quick view and list view") replaces the two top buttons; the list has a close button, and Escape returns focus to the menu. A first-visit hint ("Drag to look around · tap the glowing lights") fades after 5 s or on the first interaction (remembered in localStorage). The stand-in label shows only in development.
- **Extend route (Sep 26):** a third way to make a viewpoint for free: one continuous painting grown in Google AI Studio by extending the land picture to the right (square pictures, a third overlapping, 6 in all: picture 0, four extensions, and a closing picture that joins back onto picture 0). `scripts/pano-strip.mjs` aligns each overlap (sideways, up/down and size), matches colour, cuts along the least-different line, spreads the leftover mismatch smoothly round the loop, and wraps the painting round a cylinder with sky and ground painted beyond its edges. A picture that doesn't fit stops that viewpoint (its current panorama is kept) and is named. Tested on synthetic pictures cut from a known looping painting, some warped and recoloured to imitate Gemini redrawing the overlap: the loop length came back within 0.1%, and a wrong picture scored 0.21 against about 0.5–0.75 for real joins. Prompts in `art/pano/README.md`.
- **Day / night (Sep 26 fix):** the site theme drives both scenes live (light, dark, and System via prefers-color-scheme), through `useSiteMood` in `WorldView`. 360°: `npm run world:pano` also writes a moonlit copy of every face (`*-night.webp`, blue grade, darker starry sky), downloaded only once the dark theme is used and faded in when all six are loaded; warm window glows and fireflies are anchored in 3D. 3D photo: the same moonlight grade in the shader (0.6 s fade), a starry night sky behind, window glows and fireflies projected through the camera. Reduced motion: instant switch, fireflies still.
- **Roaming prototype (Sep 26, dropped; in git at 397c042):** a code-drawn guide robot (§2.4) walks the painted paths to wherever you click or tap (a small network of path points in `content/world-scene.ts` → `walk`; routing in `components/world/village/roam.ts`), and walking to the inn opens the projects. On wide screens the painting is drawn 25% wider than the view; drag, swipe or ← → to look around, with the sky moving at half the speed of the land. The camera follows the robot. Reduced motion: the robot steps straight to the spot and the view doesn't glide. Keyboard: the inn button opens the projects directly. Quick view and the list view are unchanged.
